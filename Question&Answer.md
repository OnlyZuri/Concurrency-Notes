...
### Answebrr
*1、除开两种不常使用或难以实现的方法（修改原始的类、继承实现扩展这个类）之外，更常用的为现有线程安全类添加新功能模块的方式是将扩展代码放入一个辅助类中，客户端进行加锁*<br>
*2、另一个更好的方法是组合，不采用辅助类那样，在客户端上给对象加锁，而是将对象封装到另一个类中，在该类中可以轻松实现同步，再提供访问途径*<br>
### Knowledge Involved
*1、在辅助类上扩展功能而进行的加锁过程中，需注意加锁对象应该准确，确保加锁对象为某个状态而不是类对象，以免该类的其他方法直接破坏其原子性*<br>
